...
class Assignment(db.Model):
    ...
    # add on
    @classmethod
    def grade_assignment(cls, p, incoming_payload):
        assignment_id = incoming_payload['id']
        assignment_grade = incoming_payload['grade']

        assignment = Assignment.get_by_id(assignment_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')

        assertions.assert_valid(p.teacher_id == assignment.teacher_id, 'Teacher ID mismatch')
        assertions.assert_valid(assignment.state == AssignmentStateEnum.SUBMITTED,
                                'Only a submitted assignment can be graded')
        assertions.assert_valid(assignment_grade in GradeEnum.__members__, 'Invalid grade')

        assignment.grade = GradeEnum(assignment_grade)
    ...
    # add on
    @classmethod
    def get_assigments_by_teacher(cls, teacher_id):
